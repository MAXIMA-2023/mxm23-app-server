...
exports.isOrganisatorPanitiaInspiceandNuntium = async (req, res, next) => {
  try {
    const { stateID } = req.params;
    const nim = req.decoded_nim;
    const organisator = await OrgDB.query().where({ nim });
    const panitia = await PanitiaDB.query().where({ nim });
    const authorizedDiv = ["D01", "D02", "D08", "D13"];
...
exports.isOrganisatorAndPanitiaInspice = async (req, res, next) => {
  try {
    const { stateID } = req.params;
    const nim = req.decoded_nim;
    const organisator = await OrgDB.query().where({ nim });
    const panitia = await PanitiaDB.query().where({ nim });
    const authorizedDiv = ["D01", "D02", "D13"];
